...
import { Code2, Cpu, Edit2, GraduationCap, Network, Plus, Save, ShieldCheck, Sparkles, Trash2, X } from "lucide-react";
import { adminFetch, responseMessage } from "@/lib/auth-client";
import type { WhyChooseUsItem } from "@/types/content";

const iconOptions = [
  { value: "Network", label: "Ekosistem", icon: Network },
  { value: "Code2", label: "Koding", icon: Code2 },
  { value: "Cpu", label: "Praktik Lab", icon: Cpu },
  { value: "ShieldCheck", label: "Karakter", icon: ShieldCheck },
  { value: "GraduationCap", label: "Karier", icon: GraduationCap },
  { value: "Sparkles", label: "Unggulan", icon: Sparkles }
];

const blankItem: Omit<WhyChooseUsItem, "id"> = {
  title: "",
  description: "",
  icon: "Network",
  highlight: "",
...
export function WhyChooseUsManager({ initialItems }: { initialItems: WhyChooseUsItem[] }) {
  const [items, setItems] = useState<WhyChooseUsItem[]>(initialItems || []);
  const [form, setForm] = useState<Omit<WhyChooseUsItem, "id">>(blankItem);
  ...
  function openEditModal(item: WhyChooseUsItem) {
    setEditingItem(item);
    setForm({
      title: item.title || "",
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await adminFetch(editingItem ? `/why-choose-us/${editingItem.id}` : "/why-choose-us", {
        method: editingItem ? "PUT" : "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(payload)
      });
      if (!response.ok) {
        throw new Error(await responseMessage(response, "Gagal menyimpan alasan memilih sekolah"));
      }
      ...
      setNotice({ type: "error", message: error.message || "Gagal menyimpan alasan memilih sekolah." });
    } finally {
      setLoading(false);
    }
  }

  async function handleDelete(item: WhyChooseUsItem) {
    if (!window.confirm(`Hapus alasan "${item.title}"?`)) return;
    setNotice(null);

    try {
      const response = await adminFetch(`/why-choose-us/${item.id}`, { method: "DELETE" });
      if (!response.ok) {
        throw new Error(await responseMessage(response, "Gagal menghapus alasan memilih sekolah"));
      }
      setItems((current) => current.filter((currentItem) => currentItem.id !== item.id));
    } catch (error: any) {
      setNotice({ type: "error", message: error.message || "Gagal menghapus alasan memilih sekolah." });
    }
  }
  ...
  return (
    <div className="grid gap-6">
      ...
      <div className="flex flex-col gap-4 rounded-[8px] border border-zinc-100 bg-white p-4 shadow-sm sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-sm font-bold text-zinc-500">{items.length} alasan tersimpan</p>
          <p className="mt-1 text-sm font-semibold text-zinc-500">Item aktif tampil pada section Why SMK Telkom Lampung di halaman utama.</p>
        </div>
        <button
          type="button"
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    ...
                    <p className="mt-2 text-sm font-bold text-rosebrand-600">{item.highlight || "Keunggulan sekolah"}</p>
                  </div>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-zinc-950/60 p-4 backdrop-blur-sm">
          <div className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-[8px] bg-white shadow-2xl">
            <div className="flex items-center justify-between border-b border-zinc-100 px-6 py-4">
              <div>
                <p className="text-xs font-extrabold uppercase text-rosebrand-600">Why SMK Telkom Lampung</p>
                <h2 className="text-xl font-black text-zinc-950">{editingItem ? "Edit Alasan" : "Tambah Alasan"}</h2>
              </div>
            ...
            <form onSubmit={handleSubmit} className="grid gap-5 overflow-y-auto p-6">
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <TextField label="Judul Alasan" value={form.title} onChange={(value) => setForm({ ...form, title: value })} required />
