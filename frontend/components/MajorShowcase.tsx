...
function MajorScrollScene({
  ...
  const imageX = useTransform(scrollYProgress, [0, 0.38, 0.7], ["0vw", "7vw", "7vw"]);
  const imageScaleX = useTransform(scrollYProgress, [0, 0.38, 0.7], [1, 0.46, 0.46]);
  const imageScaleY = useTransform(scrollYProgress, [0, 0.38, 0.7], [1, 0.78, 0.78]);
  const imageRadius = useTransform(scrollYProgress, [0, 0.38], [0, 34]);
  const imageShadow = useTransform(
    scrollYProgress,
    [0, 0.38],
    ["0 0 0 rgba(39,39,42,0)", "0 26px 90px rgba(39,39,42,0.18)"]
  );
  const overlayOpacity = useTransform(scrollYProgress, [0, 0.35], [0.58, 0.18]);
  const heroTextOpacity = useTransform(scrollYProgress, [0, 0.22], [1, 0]);
  const heroTextY = useTransform(scrollYProgress, [0, 0.22], [0, -46]);
  const contentOpacity = useTransform(scrollYProgress, [0.18, 0.42], [0, 1]);
  const contentX = useTransform(scrollYProgress, [0.18, 0.42], [74, 0]);
  const contentScale = useTransform(scrollYProgress, [0.18, 0.42], [0.96, 1]);

  return (
    <section ref={ref} id={id} className="relative h-[185vh] scroll-mt-20 bg-white" aria-label={major.name}>
      <div className="sticky top-0 h-screen overflow-hidden bg-white">
        ...
        <motion.div
          className="container-page relative z-10 grid h-full items-center lg:grid-cols-[0.52fr_0.48fr]"
          style={{ opacity: contentOpacity, x: contentX, scale: contentScale }}
        >
          <div className="hidden lg:block" />
          <div className="rounded-[8px] border border-zinc-200/80 bg-white/90 p-6 shadow-soft backdrop-blur md:p-8">
            ...
            <h2 className="mt-5 text-4xl font-black leading-tight text-zinc-900 md:text-5xl">{major.name}</h2>
            <p className="mt-5 text-lg leading-8 text-zinc-600">{major.summary}</p>

            <div className="mt-8 border-l-2 border-zinc-200 pl-6">
              <h3 className="flex items-center gap-2 text-xl font-black text-zinc-300">
                <CheckCircle2 size={21} aria-hidden />
                Fokus Kurikulum
              </h3>
              <div className="my-5 border-l-4 border-rosebrand-500 pl-5">
                <p className="text-2xl font-black text-rosebrand-600">{curriculum[0] || "Pembelajaran berbasis industri"}</p>
                <div className="mt-4 grid gap-2">
                  {curriculum.slice(1).map((item) => (
                    <p key={item} className="text-base font-bold text-zinc-500">
                      {item}
                    </p>
                  ))}
                </div>
              ...
              <div className="mt-4 flex flex-wrap gap-2">
                {careerProspects.map((item) => (
                  <span key={item} className="rounded-full bg-rosebrand-50 px-3 py-2 text-sm font-extrabold text-rosebrand-700">
                    {item}
                  </span>
                ))}
              </div>
